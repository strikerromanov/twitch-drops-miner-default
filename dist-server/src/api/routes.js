...
router.put('/settings', (req, res) => {
    try {
        ...
        const upsert = db.prepare(`INSERT INTO settings (key,value,updated_at) VALUES (@key,@value,datetime('now'))
       ON CONFLICT(key) DO UPDATE SET value=@value, updated_at=datetime('now')`);
        ...
        const bc = global.broadcast;
        if (typeof bc === 'function')
            bc({ type: 'settings_saved', settings });
...
router.get('/stats', (_req, res) => {
    try {
        res.json({
            totalAccounts: db.prepare('SELECT COUNT(*) as n FROM accounts').get().n,
            activeAccounts: db.prepare(`SELECT COUNT(*) as n FROM accounts WHERE status='farming'`).get().n,
            totalDrops: db.prepare('SELECT COUNT(*) as n FROM drops').get().n,
            claimedDrops: db.prepare('SELECT COUNT(*) as n FROM drops WHERE claimed=1').get().n,
            recentClaims: db.prepare(`SELECT COUNT(*) as n FROM point_claim_history WHERE datetime(claimed_at)>datetime('now','-24 hours')`).get().n,
            activeStreams: db.prepare('SELECT COUNT(*) as n FROM active_streams').get().n,
...
router.get('/accounts', (_req, res) => {
    try {
        res.json(db.prepare('SELECT id,username,status,createdAt,lastActive,user_id,token_expires_at FROM accounts').all());
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.post('/accounts/:id/toggle', (req, res) => {
    try {
        const a = db.prepare('SELECT status FROM accounts WHERE id=?').get(req.params.id);
        if (!a)
...
router.get('/campaigns', (_req, res) => {
    try {
        res.json(db.prepare('SELECT * FROM campaigns ORDER BY createdAt DESC').all());
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.get('/logs', (req, res) => {
    try {
        ...
        if (req.query.streamer_id) {
            conds.push('streamer_id=?');
            params.push(req.query.streamer_id);
        }
...
router.get('/betting/stats', (_req, res) => {
    try {
        const bettingSvc = global.services?.bettingSvc;
        if (!bettingSvc)
            return res.json({ error: 'Betting service not available' });
...
router.post('/betting/toggle', (req, res) => {
    try {
        ...
        const bettingSvc = global.services?.bettingSvc;
        if (!bettingSvc)
            return res.status(404).json({ error: 'Betting service not available' });
...
function getClientId(body) {
    return body?.clientId
        || db.prepare(`SELECT value FROM settings WHERE key='twitchClientId'`).get()?.value
...
router.post('/auth/device', async (req, res) => {
    try {
        ...
        if (!clientId)
            return res.status(400).json({ error: 'Twitch Client ID is required. Set it in Settings first.' });
...
router.post('/auth/device/poll', async (req, res) => {
    try {
        ...
        if (!clientId || !deviceCode)
            return res.status(400).json({ error: 'clientId and deviceCode required' });
        ...
        const existing = db.prepare('SELECT id FROM accounts WHERE user_id=?').get(userInfo.id);
        if (existing) {
...
router.post('/auth/refresh', async (req, res) => {
    try {
        ...
        if (!refreshToken || !clientId)
            return res.status(400).json({ error: 'refreshToken and clientId required' });
