...
class FollowedChannelsService {
    ...
    async indexChannels() {
        const cid = this.clientId ||
            this.db.prepare(`SELECT value FROM settings WHERE key='twitchClientId'`).get()?.value ||
            process.env.TWITCH_CLIENT_ID || '';
        if (!cid)
            return;
        const accounts = this.db.prepare(`SELECT id,username,accessToken,user_id FROM accounts WHERE status!='error' AND user_id IS NOT NULL`).all();
        for (const a of accounts) {
            try {
                const res = await fetch(`https://api.twitch.tv/helix/streams/followed?user_id=${a.user_id}&first=100`, { headers: { 'Authorization': `Bearer ${a.accessToken}`, 'Client-Id': cid } });
                if (!res.ok)
                    continue;
          ...
          `).run(a.id, stream.user_login, stream.user_id, stream.game_name, stream.viewer_count);
                ...
                logError(`[FollowedChannels] ${a.username}: ${e.message}`);
            }
        }
    }
}
