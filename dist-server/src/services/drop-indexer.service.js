...
class DropIndexerService {
    ...
    async syncCampaigns() {
        ...
        if (!clientId) {
            logWarn('[DropIndexer] No client ID');
            return;
        }
        for (const a of accounts) {
            try {
                const data = await withRetry(() => this.fetchEntitlements(a.accessToken, clientId), { retries: 3, baseDelayMs: 2000, label: `drops:${a.username}` });
                if (data?.data) {
                    for (const item of data.data)
                        this.upsertDrop(item, a.id);
                    logInfo(`[DropIndexer] Synced ${data.data.length} entitlement(s) for ${a.username}`);
                }
                break;
            }
            catch (e) {
                logError(`[DropIndexer] ${a.username}: ${e.message}`);
            }
        }
    }
    async fetchEntitlements(token, clientId) {
    ...
    async tickProgress() {
        const streams = db.prepare(`SELECT DISTINCT account_id FROM active_streams`).all();
        if (!streams.length)
            return;
        const ids = streams.map((r) => r.account_id);
      ...
      WHERE id IN (SELECT DISTINCT campaign_id FROM drops WHERE account_id IN (${ph}))`).run(...ids);
        const done = db.prepare(`SELECT d.id,d.account_id,d.campaign_id,c.name,a.username
