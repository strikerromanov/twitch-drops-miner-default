...
import { logInfo, logWarn, logDebug } from '../core/logger.js';
class ChatFarmerService {
    ...
    syncConnections() {
        const accounts = db.prepare(`SELECT id,username,accessToken,user_id FROM accounts WHERE status!='error'`).all();
        ...
        for (const a of accounts) {
            if (!this.connections.has(a.id))
                this.connectAccount(a);
        }
    }
    connectAccount(account) {
        // Get channels from both followed_channels and active_streams
        const followedChannels = db.prepare(`SELECT streamer_id FROM followed_channels WHERE account_id=? AND streamer_id IS NOT NULL`).all(account.id);
        // Also get active farming streams - now includes streamer_id
        const activeStreams = db.prepare(`SELECT streamer_id FROM active_streams WHERE account_id=? AND streamer_id IS NOT NULL`).all(account.id);
        // Merge both sources, deduplicating by streamer_id
        const channelsMap = new Map();
        for (const c of followedChannels) {
            if (c.streamer_id)
                channelsMap.set(c.streamer_id, c.streamer_id);
        }
        for (const s of activeStreams) {
            if (s.streamer_id)
                channelsMap.set(s.streamer_id, s.streamer_id);
        }
        ...
        if (!WS) {
            logWarn('[ChatFarmer] WebSocket not available in this environment');
            return;
        }
        let ws = null;
        let pingId = null;
                ...
                try {
                    ...
                    if (msg.type === 'MESSAGE') {
                        ...
                        if (inner.type === 'claim-available')
                            this.claimBonus(account, inner.data?.claim).catch(() => { });
                    }
                }
                catch { }
            };
        ...
        connect();
        this.connections.set(account.id, { disconnect: () => { alive = false; ws?.close(); if (pingId)
                clearInterval(pingId); } });
    }
    async claimBonus(account, claim) {
    ...
    clientId() {
        const r = db.prepare(`SELECT value FROM settings WHERE key='twitchClientId'`).get();
