...
import { logInfo, logError, logDebug } from '../core/logger.js';
class BettingService {
    broadcast;
    checkInterval = null;
    enabled = false;
    isActive = false;
    maxBetPercentage = 5; // Default max 5% of points
    minBetPoints = 50;
    ...
    async checkForBettingOpportunities() {
        try {
    ...
    async findAndPlaceBets(account) {
        try {
    ...
    async fetchActivePrediction(account, streamerId) {
        try {
    ...
    async evaluateAndPlaceBet(account, channel, prediction) {
        try {
            // Get or create stats for this streamer
            let stats = db.prepare('SELECT * FROM betting_stats WHERE streamer = ?').get(channel.streamer);
            if (!stats) {
    ...
    selectOutcome(outcomes, winRate) {
    ...
    async placeBet(account, channel, prediction, outcome, amount) {
        try {
            const response = await fetch('https://gql.twitch.tv/gql', {
                method: 'POST',
                headers: {
      ...
      `).run(account.id, channel.streamer, prediction.title, outcome.title, Math.round((outcome.points / prediction.outcomes.reduce((sum, o) => sum + o.points, 0)) * 100), amount, 0, // Will be updated when resolved
            'pending', this.strategy);
            // Deduct points
    ...
    getStreamerAnalysis(streamer) {
        const stats = db.prepare('SELECT * FROM betting_stats WHERE streamer = ?').get(streamer);
